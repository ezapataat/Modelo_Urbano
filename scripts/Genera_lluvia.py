...
#Funcion de prediccion de lluvia
...
def roundTime(time,each=5.0):
    ...
    return time + dt.timedelta(minutes = redondeo-time.minute)

fechaF = roundTime(time)
fechaI = fechaF - dt.timedelta(hours = 12)


# Lo convierte en texto 
fechai = fechaI.strftime('%Y-%m-%d')
fechaj = fechaF.strftime('%Y-%m-%d')
horai = fechaI.strftime('%H:%M')
horaj = fechaF.strftime('%H:%M')
dt = '5min'
verbose = '-v'
umbral_a = 0.1
umbral_b = 0.3
ruta_out = '/media/nicolas/Home/Jupyter/Esneider/Lluvia_operacional/Salidas/'

#Genera las fechas que son para el objeto
if horai <> None and horaj <> None:
    DatesSort = pd.date_range(fechai+' '+horai, fechaj+' '+horaj, freq = dt ) #freq = args.dt)
elif horai == None and horaj == None:
    DatesSort = pd.date_range(fechai, fechaj, freq = dt ) #freq = args.dt)
elif horai <> None and horaj == None:
    DatesSort = pd.date_range(fechai+' '+horai, fechaj, freq = dt ) #freq = args.dt)
elif horai == None and horaj <> None:
    DatesSort = pd.date_range(fechai, fechaj+' '+horaj, freq = dt ) #freq = args.dt)
...
for i in range(len(estaciones)):
    ...
    try:
        ...
        sentencia_pr = ("SELECT DATE_FORMAT(fecha,'%Y-%m-%d'), DATE_FORMAT(hora, '%H:%i:%s'), P1, P2, calidad FROM datos WHERE cliente= '"+estaciones[i][0]+
        ...
        Dates = [] ; pr_corr = []
        for j in pr:
            yy = int(j[0].split('-')[0]) ; mm = int(j[0].split('-')[1]) ; dd = int(j[0].split('-')[2])
            Dates.append(datetime.datetime(int(yy),int(mm),int(dd),int(j[1][0:2]),int(j[1][3:5])))
            pr_corr.append(j)
        ...
        Rain = pd.Series(Data, pd.to_datetime(Dates))
...
#Lectura de reglas 
f = open('/media/nicolas/Home/Jupyter/Esneider/Lluvia_operacional/ReglasGeneracion.bin','r')
...
print 'Genera prediccion sin problema \n'
...
for k in Predicciones.keys():
    ...
    DatesNew = [Dates[-1]+datetime.timedelta(minutes = 5*i) for i in range(0,13)]
    RainPred = pd.DataFrame(DictPred, index=pd.to_datetime(DatesNew))
    #Guardado de la consulta
    f = open(ruta_out + '_cast_'+k+'.rain', 'w')
...
for i in est_bulerias:
    ...
    print i
    print '----'
    print np.sum(cast_alto[i])
    print np.sum(cast_normal[i])
    print np.sum(cast_bajo[i])
    print '----'
